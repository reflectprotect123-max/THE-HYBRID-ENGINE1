...
1. **DONE (follow-up session, 2026-07-31)** — the engine matcher is wired into the sync flow. `planConcept2Import`/`applyConcept2Import` (packages/engine/src/concept2.ts) plan where each synced result lands — attach to a matched unlogged block, enrich an already-logged block with erg-only fields (never overwriting athlete data), or file standalone into `settings.conditioning` as `fmt: 'free'` — deduped by `externalId` with a deterministic record id (`c2-<externalId>`) so two devices importing independently merge to one record in the cloud settings union. Both Settings Concept2 cards show "Add N new results" and apply only on that explicit press. Contract check extended (110 assertions); root-level markdown excluded from its browser-facing secret scan (handoff.md itself had turned the check red on main — docs are not published, publish dir is apps/web/dist).
