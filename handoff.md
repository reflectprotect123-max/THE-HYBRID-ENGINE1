...
3. **epley ruling (user):** recommend closing as won't-fix (golden pins vanilla behavior; entry clamp already guards). Alternative: regenerate 3 golden fixture rows as a documented exception — requires explicit approval, session task #43 stays open until ruled.
