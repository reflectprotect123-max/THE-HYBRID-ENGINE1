...
const piece = (reps: string): RoundSet => ({
  exerciseIndex: 0,
  setIndex: 0,
...
describe('PieceCard — a timed piece', () => {
  it('counts down once a second', () => {
    render(<PieceCard piece={piece('3')} mode="seconds" dispatch={vi.fn()} />);
  ...
  it('can be finished early, before the clock reaches zero', () => {
    ...
    render(<PieceCard piece={piece('30')} mode="seconds" dispatch={dispatch} />);
    act(() => vi.advanceTimersByTime(1000));
  ...
  it('carries no rating — Done never asks for one', () => {
    render(<PieceCard piece={piece('30')} mode="seconds" dispatch={vi.fn()} />);
...
describe('PieceCard — a rep piece', () => {
  it('has no clock', () => {
    render(<PieceCard piece={piece('10')} mode="reps" dispatch={vi.fn()} />);
  ...
  it('dispatches completePiece on tap', () => {
    const dispatch = vi.fn();
    render(<PieceCard piece={piece('10')} mode="reps" dispatch={dispatch} />);
    fireEvent.click(screen.getByText('Done'));
    expect(dispatch).toHaveBeenCalledWith({ type: 'completePiece' });
  });
});
...
describe('PieceCard — off screen', () => {
  it("stops ticking once it leaves the block it lives on", () => {
    const dispatch = vi.fn();
    const { unmount } = render(<PieceCard piece={piece('5')} mode="seconds" dispatch={dispatch} />);
    act(() => vi.advanceTimersByTime(1000));
    unmount();
