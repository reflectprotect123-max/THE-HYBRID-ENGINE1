    ...
    <main className="min-h-screen bg-bg text-text">
      ...
      <div className="grid gap-5 p-3 sm:p-4 xl:grid-cols-[350px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-4">
          <div className="flex flex-wrap items-end gap-2"><div><p className="text-[9px] uppercase tracking-wider text-dim">Program templates</p><h2 className="text-base font-semibold">Strength and conditioning systems</h2></div><div className="ml-auto flex rounded-md border border-line2 p-0.5" role="group" aria-label="Filter Library by training system">{(['strength', 'conditioning'] as const).map((value) => <button key={value} type="button" aria-pressed={domain === value} onClick={() => setDomain(value)} className={`min-h-11 rounded px-2 text-xs capitalize ${domain === value ? 'bg-panel text-text' : 'text-muted'}`}>{value}</button>)}</div></div>
...
function Choice({ label, values, value, onChange }: { label: string; values: string[]; value: string; onChange: (value: string) => void }) {
  return <fieldset><legend className="text-xs font-medium">{label}</legend><div className="mt-1 flex flex-wrap gap-1">{values.map((item) => <button key={item} type="button" aria-pressed={value === item} onClick={() => onChange(item)} className={`min-h-11 rounded-md border px-2 text-xs capitalize ${value === item ? 'border-gold-line bg-gold-wash text-gold2' : 'border-line2 bg-panel text-muted hover:text-text'}`}>{item}</button>)}</div></fieldset>;
}
