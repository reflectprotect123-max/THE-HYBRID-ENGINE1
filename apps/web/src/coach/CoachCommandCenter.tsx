    ...
    <main className="min-h-screen bg-bg text-text">
      ...
      <section className="border-b border-line2 bg-panel3 px-3 py-2 sm:px-4" aria-label="Select client">
        <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5">
          {clients.slice(0, showAllClients ? clients.length : 3).map((client) => <button key={client.id} type="button" aria-pressed={selectedClient.id === client.id} onClick={() => selectClient(client.id)} className={`flex min-h-11 shrink-0 items-center gap-1.5 rounded-md border px-2 text-left transition-colors ${selectedClient.id === client.id ? 'border-gold-line bg-gold-wash text-text' : 'border-line2 bg-panel text-muted hover:text-text'}`}><span className="grid h-6 w-6 place-items-center rounded-full bg-well text-[9px] font-semibold">{client.initials}</span><span><span className="block text-xs font-medium">{client.name}</span><span className="block text-[9px] text-dim">{client.week}</span></span>{client.alert && <span className="ml-1 h-1.5 w-1.5 rounded-full bg-warn" aria-label="Needs attention" />}</button>)}
          <button type="button" aria-expanded={showAllClients} onClick={() => setShowAllClients((current) => !current)} className="min-h-11 shrink-0 rounded-md border border-dashed border-line2 px-2 text-xs text-muted hover:text-text">{showAllClients ? 'Show pinned' : 'All clients'}</button>
        </div>
      </section>
      ...
      <div className="grid gap-4 p-3 sm:p-4 xl:grid-cols-[minmax(0,1fr)_300px]">
        <div className="min-w-0 flex flex-col gap-5">
          <section aria-labelledby="client-overview-title">
            ...
            <nav className="mt-2 flex gap-1 overflow-x-auto text-xs" aria-label={`${selectedClient.name} details`}><span aria-current="page" className="min-h-11 shrink-0 rounded-md border border-line2 bg-panel px-2 py-1.5 text-text">Summary</span>{selectedClient.source === 'engine-local' ? <><Link to={`/coach/review/${weeklyPlan.weekStart}`} className="min-h-11 shrink-0 rounded-md border border-transparent px-2 py-1.5 text-muted hover:bg-panel hover:text-text">Week</Link><Link to="/coach/progression" className="min-h-11 shrink-0 rounded-md border border-transparent px-2 py-1.5 text-muted hover:bg-panel hover:text-text">Decisions</Link><Link to="/coach/nutrition" className="min-h-11 shrink-0 rounded-md border border-transparent px-2 py-1.5 text-muted hover:bg-panel hover:text-text">Nutrition</Link><Link to="/coach/legacy" className="min-h-11 shrink-0 rounded-md border border-transparent px-2 py-1.5 text-muted hover:bg-panel hover:text-text">Inspect details</Link></> : <span className="px-2 py-1.5 text-dim">Detailed records await the backend adapter</span>}</nav>
          </section>
