...
import { NUTRITION_LS_KEY, loadNutrition, saveNutrition } from './nutrition';

/*
 * The nutrition slice next to the engine slice, sharing a storage port and
 * nothing else. The provider is not exercised here — this suite runs in node,
 * and every rule worth pinning lives below React anyway.
 */
...
describe('nutrition storage slice', () => {
  ...
  it('hardens a hostile blob instead of trusting it', () => {
    const store = memory();
    // Foreign input reaches this path from the cloud, not only from disk.
    store.setItem(NUTRITION_LS_KEY, JSON.stringify({ logEntries: [{ id: '' }, meal('m-1')] }));
    expect(loadNutrition(store).nutrition.logEntries.map((e) => e.id)).toEqual(['m-1']);
  });
  ...
  /*
   * THE isolation these two files exist to guarantee. A nutrition write must
   * not dirty the training fingerprint (or every meal would push the whole
   * training blob), and a training write must not dirty the nutrition slice
   * (or a logged set would push the food log).
   */
