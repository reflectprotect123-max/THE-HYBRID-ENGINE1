                  ...
                  <View key={ex.id ?? ei}>
                    <ExerciseCard
                      ...
                      onAddSet={() =>
                        edit((d) => {
                          const e2 = (d.blocks[bi] as StrengthBlock<LoggedSet>).exercises[ei];
                          const ns = newSet() as LoggedSet;
                          // Inherit the exercise's own pct1rm shape, if it has
                          // one — otherwise a set added to a % mode exercise
                          // silently has no prescription: no badge here,
                          // excluded from the RPE-ramp spread, and the Logger
                          // falls through to the ordinary earned-weight path
                          // for this set alone. Cloned, not shared — same
                          // rule onPctChange follows.
                          const withPct = e2.sets.find((s) => s.pct1rm);
                          if (withPct?.pct1rm && !isWarmup(ns)) ns.pct1rm = { ...withPct.pct1rm };
                          e2.sets.push(ns);
                        })
                      }
                      onDelSet={(si) =>
                      ...
                      onModeChange={(mode) =>
                        edit((d) => {
                          ...
                          if (mode === 'seconds' || mode === 'reps') {
                            ...
                            e2.mode = 'reps_kg';
                            // ExerciseCard derives its displayed mode from the
                            // data (lo === hi reads as "flat"), so a range
                            // default of {65, 65} would immediately read back
                            // as flat and hide the just-selected range's own
                            // high-end input. Keep flat's lo/hi in lockstep
                            // but force the range's hi off of lo — the set's
                            // own existing distinct hi if there is one,
                            // otherwise lo + 5 — so a freshly picked range
                            // mode survives being read straight back.
                            e2.sets = e2.sets.map((s) => {
                              if (isWarmup(s)) return s;
                              const lo = s.pct1rm?.lo ?? 65;
                              const hi =
                                mode === 'pctFlat'
                                  ? lo
                                  : s.pct1rm && s.pct1rm.hi !== s.pct1rm.lo
                                    ? s.pct1rm.hi
                                    : lo + 5;
                              return { ...s, pct1rm: { lo, hi } };
                            });
                          }
                        })
                      }
