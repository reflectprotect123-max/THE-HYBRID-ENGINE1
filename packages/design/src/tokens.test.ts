...
import { conditioningColor, nutritionColor, strengthColor } from './tokens';
...
describe('nutritionColor', () => {
  ...
  it('does not reuse an ink the shared semantics already mean something with', () => {
    // Set<string>, not the inferred union of strengthColor's literal values:
    // the question this asks is "is this hex among those", and a Set narrowed
    // to the very literals being compared against rejects every OTHER hex at
    // compile time — including the nutrition ones the test exists to check.
    const taken = new Set<string>(SHARED_KEYS.map((k) => strengthColor[k]));
