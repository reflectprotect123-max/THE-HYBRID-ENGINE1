...
/* ---------- the BRANDED conditioning bundle ----------
 *
 * A third bundle, and the reason it has to exist is the same reason the coach
 * one does: the claim it makes is only true of a build the default one is not.
 *
 * The athlete web app was un-parked on 15 August 2026 for branded builds only
 * (`VITE_HYBRID_PRODUCT=conditioning`), which deploy to their own site. The
 * default bundle above still parks everything and the scenario at the top of
 * this file still proves it. Nothing checked the OTHER half of that fork, and
 * an un-parking that only works in a unit test is exactly the shape of failure
 * this file exists to catch — `App.tsx`'s gate, `CoachAccess`'s inversion,
 * `navTabs`'s scoping and `ManifestLink`'s swap all read the same build flag,
 * and any one of them reading it wrongly produces a site that boots and is
 * silently the wrong app.
 *
 * `dist-conditioning-smoke`, not `dist-conditioning`: the deploy directory is
 * assembled by `build:conditioning-site` and this must not overwrite it, or
 * running the checks would quietly change what a following deploy publishes.
 */
const COND_DIR = 'apps/web/dist-conditioning-smoke';
const COND_PORT = 4319;

await t('a branded conditioning bundle builds', async () => {
  execFileSync(
    'pnpm',
    ['--filter', '@hybrid/web', 'exec', 'vite', 'build', '--outDir', 'dist-conditioning-smoke', '--emptyOutDir'],
    { cwd: root, env: { ...process.env, VITE_HYBRID_PRODUCT: 'conditioning' }, stdio: 'pipe' },
  );
  assert(existsSync(resolve(root, COND_DIR, 'index.html')), COND_DIR + ' has no index.html after the build');
});

const condServer = await serve(COND_PORT, COND_DIR);
const condBase = 'http://127.0.0.1:' + COND_PORT;
const condCtx = await browser.newContext({ viewport: { width: 420, height: 880 } });
const condPage = await condCtx.newPage();
const condErrors = [];
condPage.on('pageerror', (e) => condErrors.push(String(e)));
condPage.on('console', (m) => {
  if (m.type() === 'error' && !/service worker|manifest|favicon|icon-/i.test(m.text())) condErrors.push(m.text());
});
await condPage.route('**/*.supabase.co/**', (r) =>
  r.fulfill({ status: 200, contentType: 'application/json', body: '{}' }),
);

await t('the branded bundle serves the ATHLETE app at the root, not the bench', async () => {
  await condPage.goto(condBase + '/', { waitUntil: 'networkidle' });
  // The bottom nav is the anchor because it is the one thing on every athlete
  // screen and it is generated by `navTabs`, which is the function the product
  // scoping actually runs through.
  await condPage.waitForSelector('nav[aria-label="Main"]', { timeout: 5000 });
  assert(/\/$/.test(condPage.url()), 'the root redirected away from the athlete app: ' + condPage.url());
  const txt = await condPage.textContent('body');
  assert(
    !/Sign in with your account to continue/.test(txt),
    'the branded bundle showed the coach sign-in instead of the athlete app',
  );
});

await t('STRENGTH IS NOT SURFACED — no Train tab on a conditioning build', async () => {
  // The owner's instruction, and the one claim on this build that a reader
  // would most want proven rather than asserted in a comment. `navTabs` yields
  // Home / Cond / Library / Progress / Settings here; the Train tab belongs to
  // the strength build and the unscoped one.
  const labels = await condPage.$$eval('nav[aria-label="Main"] a', (as) =>
    as.map((a) => (a.textContent || '').trim()),
  );
  assert(labels.some((l) => /Cond/i.test(l)), 'the conditioning tab is missing: ' + labels.join(', '));
  assert(!labels.some((l) => /^Train$/i.test(l)), 'a Train tab reached a conditioning build: ' + labels.join(', '));
});

await t('the coach bench is unreachable from the branded bundle', async () => {
  // `CoachAccess` inverts on a scoped build: `/coach` goes back to `/`. Without
  // it an athlete could walk into the workspace, and the manifest swap would be
  // offering the coach app on a site that has no coach in it.
  await condPage.goto(condBase + '/coach', { waitUntil: 'networkidle' });
  await condPage.waitForSelector('nav[aria-label="Main"]', { timeout: 5000 });
  assert(!/\/coach/.test(condPage.url()), '/coach did not bounce on a branded build: ' + condPage.url());
});

await t('the branded bundle offers its OWN app for install', async () => {
  // A coach manifest here installs an icon whose start_url opens a redirect —
  // the same bug this repository already shipped once in the other direction,
  // when the parked athlete manifest was still being offered.
  const href = await condPage.getAttribute('link[rel="manifest"]', 'href');
  assert(href && /manifest\.webmanifest$/.test(href), 'the branded bundle offers: ' + href);
  assert(!/coach\.webmanifest/.test(href || ''), 'the branded bundle offers the COACH app: ' + href);
});

await t('no uncaught page errors on the branded conditioning bundle', async () => {
  assert(condErrors.length === 0, condErrors.join(' | '));
});
