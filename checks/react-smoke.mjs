...
await t('a pct1rm set shows its computed %, prefills the prescribed kg, and names its source', async () => {
  const orig = await page.evaluate(() => {
    ...
    // Guard against inheriting an already-corrupted "no active session"
    // state from an unrelated, pre-existing flake earlier in this suite (a
    // scenario that can time out mid-cleanup and leave nothing marked
    // 'active') — this scenario must stand on its own regardless of what it
    // finds here, without masking or fixing that other scenario's bug.
    const origSession = db.sessions.find((s) => s.status === 'active');
  ...
  // From here through the assertions, always run cleanup — even on an early
  // throw — so this scenario cannot join the shared-state flake's corruption
  // chain (see the guard above) by leaving the shared session's status
  // clobbered for whatever runs next.
  try {
    await page.goto(base + '/log/0/0', { waitUntil: 'networkidle' });
    // Set 1 of 2 is the warm-up (`W10`) — curSetIndex lands here first. Finish
    // it in one tap (no RPE stage for a warm-up), which drops straight into the
    // rest period; skip that to reach Set 2, the pct1rm working set.
    await page.waitForSelector('button:has-text("Finish Set")');
    await page.click('button:has-text("Finish Set")');
    await page.waitForSelector('button:has-text("Skip rest")');
    await page.click('button:has-text("Skip rest")');
    await page.waitForSelector('text=Set 2 of 2');
    ...
    // only if one existed to restore (see the guard above).
    await page.evaluate((origId) => {
      const db = JSON.parse(localStorage.getItem('hybrid-engine-v1'));
...
await t('the % mode selector writes and clears pct1rm on every rated set', async () => {
  ...
  // No click needed to open the card: it is the only block (index 0) with the
  // only exercise (index 0), and Planner's `openEx` state defaults to '0-0'.
  ...
  await page.click('button:text-is("Reps")');
